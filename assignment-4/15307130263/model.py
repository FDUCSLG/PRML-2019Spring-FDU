...
class CharacterLevelCNN(nn.Module):
    # In Epoch:88/Step:7832, got best dev performance:AccuracyMetric: acc=0.485396
    def __init__(self, n_classes=20, input_length=1014, input_dim=68,
                 n_conv_filters=1024,
                 n_fc_neurons=2048):
        ...
        # trans2onehot
        self.embedding = nn.Embedding(69,68)
        self._init_embedding()

    def _init_embedding(self):
        identity_mat = np.identity(68)
        zeros = np.zeros((1,68))
        embeddings = np.append(identity_mat,zeros,0)
    ...
    def forward(self, char_data,label):
        input = self.embedding(char_data)
...
class LSTMClassifier(nn.Module):
    #In Epoch:89/Step:31506, got best dev performance:AccuracyMetric: acc=0.571163
    ...
    def forward(self, word_data, label, batch_size=None):	
        input = self.word_embeddings(word_data)
        ...
        loss = self.loss(final_output,label)

        return {'pred' : final_output,'loss' : loss}