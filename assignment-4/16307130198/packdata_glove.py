...
def transform_word2vec(dataset):
    ...
    # model = word2vec.Word2Vec(sentence_list, hs=1,min_count=1,window=5,size=256) 
    
    corpus_model = Corpus()
    corpus_model.fit(sentence_list, window=64)
...
def gain_ew(idx2word, word2vec_model):
    ...
    for index in range(v_len):
        if index != 0:
            ...
            embedding_weight[index] = word2vec_model.word_vectors[word2vec_model.dictionary[temp_word]]
